...
#include <xtlm.h>
#include "/home/luanxinya/SVD/FPGA_test/128/aie/TopGraph.cpp"
#include "../../aie_control.cpp"
#define BUSWIDTH 128

#ifdef _HYBRID_EMU_
std::map<std::string, adf::graph*> _gr_inst = { 
	{"topSweepGraph1", &topSweepGraph1},
	{"topNormGraph1", &topNormGraph1}
};
...
class PSIP_ps_i140 : public IPBlock {
	 SC_HAS_PROCESS(PSIP_ps_i140);
...
public:
	 ...
	 static PSIP_ps_i140* createInstance(sc_module_name name);
	 //  Returns the instance created using createInstance()
	 static PSIP_ps_i140* getInstance();
...
private:
	 PSIP_ps_i140(sc_module_name nm);
	 static PSIP_ps_i140* psObj;
...
PSIP_ps_i140* PSIP_ps_i140::psObj = NULL;
PSIP_ps_i140* PSIP_ps_i140::createInstance(sc_module_name name) {
	 if (psObj == NULL)
		 psObj = new PSIP_ps_i140(name);
	 return psObj;
}

PSIP_ps_i140* PSIP_ps_i140::getInstance() {
	 if (psObj != NULL) {
		 return psObj;
	 } else {
...
void PSIP_ps_i140::write32(uint64_t Addr, uint32_t Data) {
...
uint32_t PSIP_ps_i140::read32(uint64_t Addr) {
	 size_t transBytes = sizeof(uint32_t);
	 // Get the payload object
	 xtlm::aximm_payload* trans = mem_manager->get_payload();
	 trans->acquire();
...
void PSIP_ps_i140::write128(uint64_t Addr, uint32_t* Data) {
...
void PSIP_ps_i140::read128(uint64_t Addr, uint32_t* Data) {
	 size_t transBytes = 4*sizeof(uint32_t);
	 // Get the payload object
	 xtlm::aximm_payload* trans = mem_manager->get_payload();
	 trans->acquire();
...
void PSIP_ps_i140::writeGM(uint64_t addr, const void* data, uint64_t size) {
...
void PSIP_ps_i140::readGM(uint64_t addr, void* data, uint64_t size) {
    toggle_AIE_array_clk.notify(1,SC_NS);
    uint64_t remaining = size;
    uint64_t currentAddr = addr;
    unsigned char* currentDataPtr = (unsigned char*)data;
...
void PSIP_ps_i140::aximm_transaction(xtlm::xtlm_aximm_initiator_rd_socket_util& rd_util, xtlm::xtlm_aximm_initiator_wr_socket_util& wr_util, xtlm::xtlm_command command, unsigned long long address, unsigned char* pData, unsigned int trans_size_in_bytes)
{
...
PSIP_ps_i140::PSIP_ps_i140(sc_module_name nm)
  : IPBlock(nm)
...
void PSIP_ps_i140::set_payload_attr(xtlm::aximm_payload* trans, size_t transBytes) {
...
void PSIP_ps_i140::response_process() {
	 while (true) {
...
extern "C" {
	 void ess_Write32(uint64 Addr, uint Data) {
		  (PSIP_ps_i140::getInstance())->write32(Addr, Data); 
	 }
	 uint32 ess_Read32(uint64 Addr) {
		  return ((PSIP_ps_i140::getInstance())->read32(Addr));
	 }
	 void ess_Write128(uint64_t Addr, uint32_t* Data) {
		  (PSIP_ps_i140::getInstance())->write128(Addr, Data);
	 }
	 void ess_Read128(uint64_t Addr, uint32_t* Data) {
		  (PSIP_ps_i140::getInstance())->read128(Addr, Data);
	 }
	 void ess_WriteGM(uint64 addr, const void* data, uint64_t size) {
		  (PSIP_ps_i140::getInstance())->writeGM(addr, data, size);
	 }
	 void ess_ReadGM(uint64 addr, void* data, uint64_t size) {
		  (PSIP_ps_i140::getInstance())->readGM(addr, data, size);
	 }
	 IPBlock * create_ip(sc_module_name name) {
		  return (PSIP_ps_i140::createInstance(name));
	 }
	 void destroy_ip(IPBlock* ip) {
		  std::cout << "IP-INFO: deleting ip PSIP_ps_i140 " << std::endl;
		  delete ip;
	 }
}
