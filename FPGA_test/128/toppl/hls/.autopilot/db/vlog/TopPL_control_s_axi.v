...
localparam
    ...
    ADDR_S_DATA_1         = 7'h2c,
    ADDR_S_CTRL           = 7'h30,
    ADDR_CONVARRAY_DATA_0 = 7'h34,
    ADDR_CONVARRAY_DATA_1 = 7'h38,
    ADDR_CONVARRAY_CTRL   = 7'h3c,
    ADDR_ITER_DATA_0      = 7'h40,
    ADDR_ITER_CTRL        = 7'h44,
    WRIDLE                = 2'd0,
    WRDATA                = 2'd1,
    WRRESP                = 2'd2,
    WRRESET               = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CONVARRAY_DATA_0: begin
                    rdata <= int_ConvArray[31:0];
                end
                ADDR_CONVARRAY_DATA_1: begin
                    rdata <= int_ConvArray[63:32];
                end
...
assign U             = int_U;
assign S             = int_S;
assign ConvArray     = int_ConvArray;
...
always @(posedge ACLK) begin
    if (ARESET)
        int_ConvArray[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONVARRAY_DATA_0)
            int_ConvArray[31:0] <= (WDATA[31:0] & wmask) | (int_ConvArray[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_ConvArray[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CONVARRAY_DATA_1)
            int_ConvArray[63:32] <= (WDATA[31:0] & wmask) | (int_ConvArray[63:32] & ~wmask);
    end
end
